...
2. 자료 :
3. 조건(집합s) :
- 하강 경로 : 첫번쨰행 임의의 요소에서 시작해서, 바로 아래 또는 대각선으로 왼쪽/오른쪽에 있는 다음 행의 요소
- 최소

# 2. 계획 - 포기는 자료로부터 유의미한 진행이 불가하다면
- 시간 복잡도 상한(3억, 1억^8) : 100*100 이면 다 더해도 10^4 뭐 뭐...
- int overFlow 가능성(합을 구해야하는가) : O -> 다만, 10^4라 굳이 안해도 상관없긴함
1. 유사한 문제 : 
2. 유사한 문제와 다른 부분 :
3. 문제를 다르게 진술하기 : 
4. 선행(근접조건 제거) :
   - 위, 대각선 위의 왼쪽/오른쪽 중 최소를 찾아서 혅대 숫자에 더합니다. 
   - min을 넣어주고, 마지막라인 만들떄만 min을 기록해두자
...
https://leetcode.com/problems/minimum-falling-path-sum/submissions/1832783086