...
2. 자료 : m x n -> 행렬
3. 조건(집합s) :
- (0,0)에서 (m-1, n-1)로 가는 경로
- 경로상의 모든 숫자의 합을 최소화
- 아래나 오른쪽으로만 이동

# 2. 계획 - 포기는 자료로부터 유의미한 진행이 불가하다면
- 시간 복잡도 상한(3억, 1억-8) : 상관 없을듯 
- int overFlow 가능성(합을 구해야하는가) : 합이 나오진함
1. 유사한 문제 : 
2. 유사한 문제와 다른 부분 :
3. 문제를 다르게 진술하기 : 
4. 선행(가까운 조건 제거) :
   - 왼쪽 위쪽에서 온 것중에 작은거 선택해서 현재 숫자에 +
...
- 기본값보다 더 큰 값이 나왔을떄 터지네
- https://leetcode.com/problems/minimum-path-sum/submissions/1831452491