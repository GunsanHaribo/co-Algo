...
# 1. 이해
1. 미지(어절) : 취할 수 있는 가능한 고유 경로의 수 -> 고유 경로?(같은 칸이라도)
2. 자료 :  m(세로) x n(가로) 격자 -> bound 필요ㅕ 
3. 조건(집합s) : 
- 아래쪽과 오른쪽으로만 이동가능
- 0,0 -> m-1,n-1

# 2. 계획 - 포기는 자료로부터 유의미한 진행이 불가하다면
- 시간 복잡도 상한(3억, 1억-8) : 10^4
- int overFlow 가능성 :
1. 유사한 문제 : 
2. 유사한 문제와 다른 부분 :
3. 문제를 다르게 진술하기 : 
4. 선행(가까운 조건 제거) :
   - 오른쪽 아래쪽 두가지만 가능
   - 오른쪽 아래쪽의 수를 더해서 현재에 도달할 수 이는 경우 가져오기
...
- 그냥 BFS랑 다른게 뭘까?, 다르긴하지,, 가능한 수인데
- https://leetcode.com/problems/unique-paths/submissions/1831417650